...
public class RegistrationCacheModel
	...
	public Registration toEntityModel() {
		...
		registrationImpl.setRegistrationResourceId(registrationResourceId);
		registrationImpl.setUserId(userId);
		registrationImpl.setAuthorId(authorId);

		if (registrationTime == Long.MIN_VALUE) {
			registrationImpl.setRegistrationTime(null);
		}
		else {
			registrationImpl.setRegistrationTime(new Date(registrationTime));
		}
