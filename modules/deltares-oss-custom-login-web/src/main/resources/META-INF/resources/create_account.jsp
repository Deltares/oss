...
List nonPrivateSites = (List) request.getAttribute("nonPrivateSites");
...
long guestGroupId = ParamUtil.getLong(request,"guestGroupId");
%>
...
<aui:form action="<%= createAccountURL %>" method="post" name="fm">
	...
	<aui:fieldset column="<%= true %>">
		<aui:col width="<%= 75 %>" >
			...
			<aui:input autoFocus="<%= true %>" name="firstName">
				<aui:validator name="required" />
			</aui:input>
			
			<aui:input autoFocus="<%= true %>" name="middleName" />
			
			<aui:input autoFocus="<%= true %>" name="lastName">
				<aui:validator name="required" />
			</aui:input>
						<%
			...
			<aui:input id="organization" name="organization" type="text" value=""/>
			
			<aui:input type="checkbox" name="orgAdmin" label="organizational-admin" value=""/>
			
			<c:if test="${nonPrivateSites.size() > 0}">
				<aui:fieldset label="select-community-section" >
					<c:forEach items="${nonPrivateSites}" var="site">
						<c:if test="${guestGroupId != site.getGroupId() }">
							<aui:input type="checkbox" value="${site.getGroupId()}" name="communityId_${site.getGroupId()}" label="${site.getName()}" />
						</c:if>
					</c:forEach>
				</aui:fieldset>
			</c:if>
